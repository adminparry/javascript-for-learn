...
// seajs配置开始
seajs.config({
	base:'file:///Users/parry/OTHERS/seajs/static/js',
	alias:{
		'jquery':'lib/jquery',
		'one':'lib/1',
		'two':'lib/2',
...
seajs.use('jquery',function(){
	// 加载全站方法
	// seajs.use('module/init');
	// 判断是否支持html5
	if(!!window.AnimationEvent){
		
	}
	var _module = $('body').attr('module'),_page = $('body').attr('page');
	if(_module){
		seajs.use('module/'+ _module +'/init');
	}else{
		return false;
	};
	//层级下还有
	if(_module && _page){
		seajs.use('module/'+ _module + '/'+ _page +'/init');
	}else{
		return false;
	};
	console.log('module/'+ _module +'/init', 'module/'+ _module + '/'+ _page +'/init')
